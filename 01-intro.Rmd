...
Esta relación nos da cuántas veces una cantidad es igual a la otra cantidad, como 6 estudiantes por 1 profesor o 2 tazas de azúcar para 5 tazas de harina. Independientemente de la situación, las comparaciones de razones siempre se relacionan de forma multiplicativa. Por ejemplo, la razón de 6 estudiantes por 2 adultos es que hay 3 veces más estudiantes que adultos.  También se  puede describir esta relación de varias formas, tales como:

* 3 estudiantes por cada adulto.
* Un adulto por cada 3 estudiantes. 
* El número de adultos es $\frac{1}{3}$ del número de estudiantes. 

Independientemente de la forma en que se comunique esta relación, debe ser claro que hay 3 veces más estudiantes que adultos.
...
![Sin titulo](C:/Users/USUARIO/Pictures/arroz.png){width=50px} ![Sin titulo](C:/Users/USUARIO/Pictures/harina.png){width=40px}
...
Con lo que Ana de 17 preguntas acertó en 10 en los dos parciales.
...
\end{equation}$$
lo que nos dice es una razón. Estos tipos de razones que involucran un periodo de tiempo los llamaremos *tasas*.

## Proporción
Matematicamente, una proporción es la igualdad de dos razones cuyo objetivo es establecer la relación entre una parte con respecto al todo, en otras palabras es la relación que se establece entre un subconjunto $n$ de un conjunto universo $N$ y se define como
$$\begin{equation}
...
| ---------------------------------|-------------------------------------------|------------------------|
|Porporción de matriculados extranjeros de pregrado en la universidad en el periodo $2020-1$ | Número de  matriculados extranjeros de pregrado en la universidad en el periodo $2020-1$ /Total de estudiantes matriculados de pregrado en el periodo $2020-1$ | $\frac{300}{36358}=0,008$ |
|Proporción de matriculados de pregrado en el área de ciencias sociales y humanas de la universidad en el periodo $2020-1$. |Número total matriculados de pregrado en el área de ciencias sociales y humanas en el periodo 2020-I/Total de estudiantes matriculados de pregrado en el periodo $2020-1$ | $\frac{5946}{36358}=0,16$
| Proporción de matriculados de pregrado en el área de matemáticas y ciencias naturales de la universidad en el periodo $2020-1$. |Número total matriculados de pregrado en el área de ciencias sociales y humanas en el periodo 2020-I/Total de estudiantes matriculados de pregrado en el periodo $2020-1$ | $\frac{3863}{36358}=0,10$ |
...
En epidemiología, las proporciones permiten expresar la frecuencia de los eventos de salud, la enfermedad y las lesiones, en términos de su incidencia o prevalencia; es decir entre los casos nuevos o los casos existentes ocurridos. También permite mostrar en qué medida una población está expuesta a un
determinado factor de riesgo.

|Ejemplo. *Proporción anual de muertes en una población* |
|--------------------------------------------------------------------|
|Si en un año en una población de 450 habitantes se presentaron 10 muertes en un año.Entonces <br> $$\text{Proporción de anual de muertes}=\frac{\text{Número de personas fallecidas en el año}}{\text{Población total}}=\frac{10}{450}=0,02$$ |
...
El porcentaje se representa con el símbolo $\%$, donde el conjunto $N$ representa el $100$ por ciento, y cada una de las relaciones obtenidas al dividir entre el total (elementos de $N$) y multiplicarlo por cien representa un tanto de cien, y es definido como tanto por ciento. 
...
En definitiva, un porcentaje es un ejemplo de proporcionalidad. 
...
La unidad de medida empleada se conoce como *tiempo-persona* de seguimiento. Por ejemplo, la observación de 100 individuos libres del evento durante un año corresponde a 100 años-persona de seguimiento; de manera similar, 10 sujetos observados durante diez años corresponden a 100 años-persona. Entonces, la población $n$ en observación durante un año corresponde a $n$ año-persona. Así que, esto explica el por qué los ejemplos clásicos sobre tasas (algunos se presentarán a continuación) que corresponden a estudios hechos durante un período de un año, por ende, la sumatoria de los períodos donde los individuos de la población libres del evento estuvieron expuestos al "riesgo" en el periodo de un año (año-persona) coindice con la población en observación.
...
* *Tasa de desempleados:* Mide el nivel de desocupación en relación con la población activa por cada $10^n$ habitantes en un año. <br>
...
El concepto de tasa es similar al de una proporción, como bien se dijo, con la diferencia de que las tasas llevan incorporado el concepto de tiempo haciendo referencia a la unidad de medida empleada tiempo-persona. Para comprender mejor como actua dicha diferencia, consideremos la proporción de incidencia vs. la tasa de incidencia de una determinada población.
...
|Ejemplo. *Porporción de incidencia de covid-19 en una población*|
|--------------------------------------------------------------------|
|Una población de 10.000 habitantes, registró 1.800 nuevos casos del covid en tres meses. <br> Proporción de incidencia $\times 1.000=\frac{1.800}{10.000}\times 1.000=180$ por mil.|
...
|*1). Período mensual*|Mes 1|Mes 2|Mes 3|Total 3 meses|
|----------------------|----------|------------|-----------|-----------|
|*2). Casos nuevos de Covid-19*| 200 |600 | 1.000|1.800|
|*3). Nuevas entradas a la cohorte*[^1]|100| 500| 1.100|1.700|
|*4). Pérdidas en el seguimiento*|100|300|800|1.200|
|*5). Movimiento en el período* $=+3)-2)-4)$|-200|-900|-700|-1.300|
|*6). Persona al principio del período*|10.000|9.800|8.900|10.000|
|*7). Personas al final del período* $=6)+5)$|9.800|8.900|8.200|8.700|
|*8). Tiempo-persona en meses-persona* $=7)/2)$|4.900|4.450|4.100|13.450|
|*9). Tasa por 1.000* $=2)/8)\times 1.000$|40,8|134,8|243,9|133,8|
...
>*3). Nuevas entradas a la cohorte*: Consta de las nuevas entradas a la
...
Un índice (número índice) es una medida estadística diseñada para estudiar las variaciones de una magnitud o más de una en relación con el tiempo o el espacio. Particularmente, el número índice recoge la evolución relativa en el periodo $t$ de una magnitud económica (precios, producciones, …) de un conjunto de bienes o productos respecto de un periodo base ó de referencia $0$. También permite comparar una magnitud económica en una zona geográfica respecto de una zona de referencia. Ahora, por simplicidad, sólo se considerará el caso de magnitudes temporales.

Al período inicial se le denomina período base o referencia y se le asigna el valor $100$ y se compara con el periodo actual o también llamado corriente.  Así si definimos a $I_{0}^{t}$ como el número índice de un determinado valor en el período $t$, respecto al período base $0$.
...
Sería deseable que todo numero índice satisfaciera las siguientes propiedades:
...
|***4. Inversión:*** Si permutamos el periodo base y el actual, el índice que se obtiene es el inverso del original.$$I_{t/0}=\frac{x_t}{x_0}=\frac{1}{\frac{x_0}{x_t}}=\frac{1}{I_{0/t}}\longrightarrow I_{t/0}I_{0/t}=I_{t/t}=1$$| 
...
Hablaremos de *índices complejos* cuando se forman a partir de los índices simples. Su función principal será sintetizar o resumir la información que proporcionan los índices simples que entran en su composición.
...
Los índices sintéticos representan una medida que se obtiene mediante la agregación adecuada de un conjunto de indicadores sociales. Su elaboración no es sencilla, ya que los indicadores son de distinta naturaleza y provienen de diferentes fuentes de datos. En este sentido, en la literatura se recomienda seguir distintas etapas para su elaboración: *selección de los indicadores*, *selección de la técnica de estandarización, ponderación* y *agregación*. 

*1. Selección de los indicadores*: En esta primera etapa se deberá seleccionar los indicadores, los cuales llamaremos indicadores parciales, esto perimite marcar la ruta hacia lo que queremos medir. Estos indicadores pueden ser, a su vez, tasas, proporciones, razones o índices. 

*2. Selección de la técnica de estandarización:* Los indices sociales se miden en distintas unidades de medida: años, personas, porcentajes, etc. La estandarización permite que un conjunto heterogéneo de indicadores pueda ser transformado a una unidad de medida común.

*3. Técnica de ponderación*: No se puede dar la misma importancia a
todos los indicadores y, por ende, es necesario ponderarlos.

*4. Selección del método de agregación:* En esta etapa se busca unificar los indicadores en un índice sintético a través de algún procedimiento estadístico, matemático o lógico. Resulta deseable que el valor que arroje la medida final represente niveles factibles de ser interpretados en una escala  de alto, medio, bajo o nulo. Esto último dependerá de la técnica de estandarización utilizada y de que el procedimiento de agregación no haga perder esa propiedad. La agregación se puede efectuar como la media aritmética ponderada, o bien,
como la media geométrica poderada. La primera es la más utilizada y sencilla en su elaboración.

Para ilustrar lo anterior, tomaremos el índice sintético de bienestar social y veremos como se calcula a grandes rasgos. El índice sintético de bienestar social (IBS) se mide con seis índices parciales: índice de salud, seguridad ciudadana, educacíon, trabajo, vivienda e ingresos relativos y estos a su vez, se subdividen en los siguientes índices simples.
...
Es decir, que para realizar la síntesis final partimos de una matriz de datos que contiene la valoración o el puntaje obtenido en cada uno de estos seis índices parciales. Porteriormente, a través de los  métodos de agregación como la media ponderada, generamos el
número de rankings. En el caso hipotético, asignaremos a salud y
seguridad un peso del $15\%$, a trabajo  un $30\%$, a vivienda un $20\%$ y a ingresos y educación le introducimos un $10\%$. 
...
Es un índice, calculado por el Icfes, que mide cuatro
aspectos de la calidad de la educación en todos los colegios
del país: Progreso, Desempeño, Eficiencia y Ambiente Escolar. El ISCE está en una escala de 1 a 10, en donde Progreso y Desempeño pesan 40 por ciento, cada uno, y Eficiencia y Ambiente Escolar pesan 10 por ciento, cada uno. Para el caso de la Media, el componente Eficiencia pesa el 20 por ciento.
...
[documentopbm](https://planeacion.medellin.unal.edu.co/images/documentos/EvaluacionMatriculasUnalMed.pdf)
